...
4. Ubuntu 22.04版本
    ...
    通过nvidia-settings修改显示器亮度

    `sudo apt install nvidia-settings`

    ![](https://pictures-kiana.oss-cn-beijing.aliyuncs.com/img/202208131700411.png)
...
1. [ros2 topic](https://www.guyuehome.com/10560)
...
2. 字节发送
    ...
    ```c
    obstacle_msg.buf[i * 6] = center_r & 0XFF;    //
    obstacle_msg.buf[i * 6 + 1] = (center_r>>8) & 0XFF;
    obstacle_msg.buf[i * 6 + 2] = center_x & 0XFF;    //
    obstacle_msg.buf[i * 6 + 3] = (center_x>>8) & 0XFF;
    obstacle_msg.buf[i * 6 + 4] = center_y & 0XFF;    //
    obstacle_msg.buf[i * 6 + 5] = (center_y>>8) & 0XFF;
    
    int Currentvalue =(short)((obstacle_msg.buf[3] << 8) +obstacle_msg.buf[2]);    
    ```
...
3. topic监视
    ...
    ![](https://pictures-kiana.oss-cn-beijing.aliyuncs.com/img/202208101622304.png)

4. 想骂人，，，

    1. [ros时间](https://answers.ros.org/question/287946/ros-2-time-handling/)

        [函数标准](docs.ros2.org/ardent/api/rclcpp/classrclcpp_1_1_node.html#af706b231620f1c120b7ccd738ec31867)

    2. 重启，莫名就好了？

    3. 我是憨憨

        ![](https://pictures-kiana.oss-cn-beijing.aliyuncs.com/img/202208102135067.png)

        marker踩坑：
        
        1. 接收时没转为浮点数，导致半径0.6m被认为时0，不显示地图

            `float r =(short)((msg_rec.buf[6 * id + 1] << 8) +msg_rec.buf[6 * id + 0])/100.;`

            同时注意lidar_pkg中将数据*100，全部以cm为单位，避免因为小数点后太多位可能造成运算错误

            using_marker中发送全部是以send_link为基准，平时都习惯右边
            轴前方y，但实际上ROS坐标系如下图，有一个90°旋转
            ...
            tf_node = Node(
                package = "tf2_ros", 
                executable = "static_transform_publisher",
                arguments = ["0", "0", "0", "1.57079", "0", "0", "send_link", "laser"])     ## 修改此处            
            ```
        2. 接收雷达数据是一定要注意数据个数
...
2. Remote-SSH
    ...
    1. [安装docker](https://docs.docker.com/engine/install/ubuntu/#prerequisites)

    2. [vscode配置](https://www.cnblogs.com/Super-why/p/15661862.html#:~:text=%E7%A1%AE%E4%BF%9Dubuntu%E7%B3%BB%E7%BB%9F%E5%B7%B2%E7%BB%8F%E5%AE%89%E8%A3%85%E5%A5%BDSSH%E6%9C%8D%E5%8A%A1%E7%AB%AF%EF%BC%8C%E6%B2%A1%E6%9C%89%E5%AE%89%E8%A3%85%E5%8F%AF%E4%BB%A5%E5%8F%82%E8%80%83%20%5BUbuntu%E7%8E%AF%E5%A2%83%E4%B8%8BSSH%E7%9A%84%E5%AE%89%E8%A3%85%E5%8F%8A%E4%BD%BF%E7%94%A8%5D%20%E5%9C%A8VScode%E9%A1%B5%E9%9D%A2%E4%BD%BF%E7%94%A8%E5%BF%AB%E6%8D%B7%E9%94%AE%20ctrl%2Bshift%2Bp%20%E8%BE%93%E5%85%A5,Remote-SSH%20%E9%80%89%E6%8B%A9%20Remote-SSH%3AAdd%20New%20SSH%20Host)