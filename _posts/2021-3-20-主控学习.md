...
1. 有一个末端速度，末端速度为0时进入锁位置模式，也可能不为0紧跟着切向速度。
2. 速度理论上是一个先加速再减速的过程（梯形），必然存在加速时加速度，减速时加速度，匀速时速度。问过胡凯鹏，我们对于加速是给定加速度，减速是给定减速距离（据说可以看之前学长代码）加速距离和减速距离就靠手搓，，，**只要轮子不打滑**加速度可以给很大
